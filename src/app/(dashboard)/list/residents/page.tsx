...
import { Room, Prisma, Resident } from "@prisma/client";
import Image from "next/image";
import Link from "next/link";

import { auth } from "@clerk/nextjs/server";

type ResidentList = Resident & { room: Room };

const ResidentListPage = async ({
  searchParams,
}: {
  ...
  const renderRow = (item: ResidentList) => (
    <tr
      key={item.id}
    ...
    >
      <td className="flex items-center gap-4 p-4">
        ...
        <div className="flex flex-col">
          ...
          <p className="text-xs text-gray-500">
            {item.room?.name || "No Room Assigned"}
          </p>
        </div>
      </td>
      ...
      <td>
        <div className="flex items-center gap-2">
          <Link href={`/list/residents/${item.id}`}>
            <button className="w-7 h-7 flex items-center justify-center rounded-full bg-lamaSky">
              <Image src="/view.png" alt="" width={16} height={16} />
            </button>
          </Link>
          {role === "admin" && (
            <FormContainer table="resident" type="delete" id={item.id} />
          )}
        </div>
      </td>
    </tr>
  ...
  const [data, count] = await prisma.$transaction([
    ...
    prisma.resident.count({ where: query }),
  ]);
